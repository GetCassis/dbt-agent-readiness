...
- {if inventory.packages_unresolved: **Package deps not installed.** `packages.yml` / `dependencies.yml` is declared but `dbt_packages/` is absent, so {inventory.packages_unresolved_ref_count} refs to package models or user-supplied extension points could not be resolved. They were NOT reported as broken refs (they would be false positives). Run `dbt deps` (and `dbt compile`) before a second audit for reliable broken-ref and column detection.}
